...
- `Resources.runtime = "vm"` asks for a sandbox with its own kernel, for example to run Docker with overlay mounts. Modal creates a VM sandbox. Daytona rejects it, because its snapshot sets VM or container.
