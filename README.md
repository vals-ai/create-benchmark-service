...
Sandbox setup and live sandbox evaluation accept a request-scoped `sandbox_provider` discriminator so one hosted service can use different sandbox providers per request. Daytona uses `{"type": "daytona", "api_key": "…", "api_url": "…", "target": "…"}`; Modal config is recognized as `{"type": "modal"}`, but its provider adapter still needs a real implementation. Callers can validate provider-neutral config dictionaries with `sandbox_provider_config_from_mapping(...)`. If `sandbox_provider` is omitted, the service falls back to the legacy Daytona headers: `x-sandbox-provider`, `x-api-key`, `x-api-url`, and `x-target`. Live evaluation accepts `{"task_id": "…", "instance_id": "…", "sandbox_provider": {...}, "dataset": "…"}`. Eval-only retry uses `/ws/evaluate-response` with `{"task_id": "…", "eval_resume_state": {...}, "dataset": "…"}` and does not require a sandbox provider.
...
- **`SetupTaskRequest`** / **`EvaluateInstanceRequest`** — `task_id`, `instance_id`, optional `sandbox_provider`, `dataset`
