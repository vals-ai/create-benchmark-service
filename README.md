...
- Durable storage is requested with `Resources.volumes`. Modal supports named volumes, optional creation, read-only mounts, and run-scoped subpaths. Daytona rejects nonempty volume requests until its different provisioning semantics are implemented; requested storage is never silently omitted.
- `SandboxVolume.sub_path_template` scopes one named volume across runs. `{benchmark_id}` reads the `Id` label (falling back to `run-id`), `{task_id}` reads the `Task` label, and `{sandbox_name}` is the provider-facing sandbox name; a template referencing identity the labels do not carry fails the create rather than mounting a shared subdirectory. Mounts are applied only when a sandbox is created — a retry that reuses a still-running sandbox keeps the mounts it was created with.
