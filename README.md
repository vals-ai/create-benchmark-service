...
**`GET /v1/datasets/{dataset}/tasks/{task_id}/inputs`** — return files that must be placed in the generation sandbox for one task. Benchmarks opt in by overriding `list_task_inputs` and returning `TaskInput(filename, dest)` entries. `filename` is the plain file name used by the download endpoint; `dest` is the absolute sandbox path where the lab orchestrator places it. The default returns an empty list for prompt-only tasks.

**`GET /v1/datasets/{dataset}/tasks/{task_id}/inputs/{filename}`** — download one declared input as `application/octet-stream`. Benchmarks serving inputs also override `task_input_path` to resolve declared names to local files. The framework checks Descope authentication, dataset access, dataset and task existence, and the task's declaration before resolving the file. Filenames are safe basenames and may contain spaces and punctuation; clients URL-encode them as one path segment. Destinations must be normalized absolute POSIX file paths without parent traversal. Unknown tasks and undeclared files return 404. These endpoints are not available to trial tenants.
