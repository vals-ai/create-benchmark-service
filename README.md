...
- Durable storage is requested with `Resources.volumes`, and both providers mount named volumes with optional creation and run-scoped subpaths. Modal maps each mount through `Volume.from_name` plus `with_mount_options`. Daytona resolves each volume through its volume service and waits for the volume to reach `ready`, because a volume is briefly unmountable right after it is provisioned. `create_if_missing` provisions the volume on first use; without it, a missing volume fails the create with a `SandboxError` rather than a `SandboxNotFoundError`, which would read as a missing sandbox.
- `read_only` is Modal-only. Daytona mounts every volume read-write, so it rejects a read-only request instead of silently granting write access to a volume the benchmark asked to protect.
- Volumes mount on snapshot-backed sandboxes on both providers. A snapshot fixes its sizing, not what is mounted into it, so `eval_sandbox.resources` may declare `volumes` for a snapshot-backed grading sandbox; overriding any sizing field still fails.
- `SandboxVolume.sub_path_template` scopes one named volume across runs. `{run_id}` reads the `Id` label (falling back to `run-id`) and `{task_id}` reads the `Task` label; both identify a single run, so a subpath built from them is per-run storage, not a benchmark-wide cache. A template with no placeholders is a literal subpath. A template referencing identity the labels do not carry fails the create rather than mounting a shared subdirectory. A read-only mount cannot create its own subpath, so a subpath under a read-only mount must already exist. Mounts are applied only when a sandbox is created — a retry that reuses a still-running sandbox keeps the mounts it was created with.
...
- **`Resources`** — `vcpu`, `memory`, `disk`, optional `gpu` (count, default 0), `gpu_type` (requires `gpu >= 1`), and provider-neutral `volumes`
- **`SandboxVolume`** — provider volume `name`, absolute `mount_path`, optional `read_only`, `create_if_missing`, and `sub_path_template`
