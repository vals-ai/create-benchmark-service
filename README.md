...
- A task that needs its own kernel (for example, Docker with overlay mounts) sets `Resources.runtime = "vm"`. Modal honors it with a VM sandbox over the provider's configured runtime. Daytona takes VM or container from the snapshot's sandbox class, so it rejects the request instead of returning a container.
...
- **`Resources`** — `vcpu`, `memory`, `disk`, optional `gpu` (count, default 0) and `gpu_type` (requires `gpu >= 1`), optional `runtime` (`"vm"` when the sandbox needs its own kernel)
