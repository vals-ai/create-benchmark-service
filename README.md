...
| `GET` | `/v1/datasets/{dataset}/tasks` | Lab-facing dataset task list (see [v1 Task Discovery](#v1-task-discovery-lab-facing) below) |
...
### v1 Task Discovery (lab-facing)

Lab-facing generation uses `/v1/datasets/{dataset}/tasks` for task discovery.
Evaluation and scoring use the internal `/evaluate-response/` and
`/final-score/` endpoints directly.
...
**Deferred to follow-on plans.** `GET /v1/schema` and `GET /v1/tasks/{task_id}` (single-task lookup).
