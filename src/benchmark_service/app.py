...
class BenchmarkServiceApp(FastAPI):
    ...
    async def _verify_task_ids(
        ...
        filtered_task_ids = await self.service.filter_tasks(task_filter)
    ...
    async def _retrieve_task(
        ...
        return await self.service.retrieve_task(task_id, skip_validation)
    ...
    async def _setup_task(self, websocket: WebSocket) -> None:
        ...
        try:
            ...
            async with AsyncDaytona(config=daytona_config) as daytona:
                ...
                async for message in self.service.setup_task(request.task_id, sandbox):
    ...
    async def _evaluate_response(self, request: EvaluateResponseRequest) -> Any:
        return await self.service.evaluate_response(request)
    ...
    async def _evaluate_instance(self, websocket: WebSocket) -> None:
        ...
        try:
            ...
            async with AsyncDaytona(config=daytona_config) as daytona:
                ...
                async for message in self.service.evaluate_instance(request.task_id, sandbox):
                    await websocket.send_json(message.model_dump())
    ...
    async def _final_score(self, request: FinalScoreRequest) -> FinalScoreResponse:
        ...
        validated_task_ids = await self.service.validate_task_ids(tasks_evaluated)
        result = await self.service.calculate_final_score(request.evaluation_results)
