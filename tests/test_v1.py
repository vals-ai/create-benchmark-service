"""Tests for the remaining lab-facing /v1/ task-discovery surface."""
...
def test_v1_eval_and_score_routes_are_removed(descope_client: TestClient) -> None:
    headers = {"x-descope-api-key": "key-acme"}

    eval_resp = descope_client.post("/v1/evaluate", json={}, headers=headers)
    score_resp = descope_client.post("/v1/score", json={}, headers=headers)
...
def test_v1_list_dataset_tasks_unauthorized_nonexistent_dataset_gets_403_not_404(
...
def test_v1_list_dataset_tasks_404_for_unknown_dataset_in_allowlist(monkeypatch: pytest.MonkeyPatch) -> None:
    """If the tenant is allowed to access dataset X but X is not in load_datasets,
    return 404 rather than leaking the difference between not allowed and missing."""
    clear_allowlist_cache()
    clear_auth_cache()
    monkeypatch.setenv("AUTH_REQUIRED", "true")
    monkeypatch.setenv("DESCOPE_PROJECT_ID", "P_test")
